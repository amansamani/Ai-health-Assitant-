...
function ProfileInput({ label, icon, value, onChangeText, keyboardType = 'numeric', placeholder, unit }) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSub}>Help us personalize your plan</Text>
      </View>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          <Animated.View entering={FadeInDown.delay(80)} style={styles.card}>
            <View style={styles.row2}>
              <ProfileInput label="Age" icon="calendar-outline" value={form.age} onChangeText={v => set('age', v)} placeholder="25" unit="yrs" />
              <ProfileInput label="Height" icon="resize-outline" value={form.height} onChangeText={v => set('height', v)} placeholder="170" unit="cm" />
            </View>
            <ProfileInput label="Weight" icon="scale-outline" value={form.weight} onChangeText={v => set('weight', v)} placeholder="70" unit="kg" />
          ...
          <Animated.View entering={FadeInDown.delay(240)} style={styles.card}>
            <ProfileInput label="Diseases (optional)" icon="medkit-outline" value={form.diseases} onChangeText={v => set('diseases', v)} keyboardType="default" placeholder="diabetes, hypertension" />
            <ProfileInput label="Allergies (optional)" icon="warning-outline" value={form.allergies} onChangeText={v => set('allergies', v)} keyboardType="default" placeholder="peanuts, lactose" />
          </Animated.View>

          <TouchableOpacity style={[styles.saveBtn, loading && { opacity: 0.6 }]} onPress={handleSave} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" size="small" /> : (
              <><Ionicons name="checkmark-circle" size={20} color="#fff" /><Text style={styles.saveText}>Save Profile</Text></>
            )}
          </TouchableOpacity>
        </ScrollView>
