...
export default function ProfileScreen() {
  ...
  const loadProfile = useCallback(async () => {
    try {
      setLoading(true);
      const [profileRes, currentToken, requestRes] = await Promise.all([
      ...
      setPendingFollowRequests(Number(requestRes?.data?.total || 0));
    } catch (err) {
        ...
        <FadeSlideIn delay={100}>
          ...
          {profile.profileVisibility === "private" && (
            <ProfileRow
              ...
              subtitle={pendingFollowRequests > 0 ? `${pendingFollowRequests} pending request${pendingFollowRequests === 1 ? "" : "s"}` : "Manage requests from people who want to follow you"}
